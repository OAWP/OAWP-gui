...
char* findUiFile(const char *restrict filename) {

    #ifdef DEBUG
    const char xdg_data_dirs[] = "./assets/";
    #else
    const char *xdg_data_dirs = getenv("XDG_DATA_DIRS");
    #endif

    char* dirs = g_strdup(xdg_data_dirs);
    char* dir  = strtok(dirs, ":");

    while (dir != NULL) {
    #ifdef DEBUG
        log_info("This is a debug build specific for development purposes and only work UI files from `./assets/ui/'");
        char *filepath = g_strdup_printf("%s/ui/%s", dir, filename);
    #else
        char *filepath = g_strdup_printf("%s/oawp-gui/ui/%s", dir, filename);
    #endif
...
void activate(GApplication *app, gpointer user_data) {
  ...
  if(gtk_builder_add_from_file(builder_main, findUiFile("main.ui"), &error)   == 0 ||
     gtk_builder_add_from_file(builder_popup, findUiFile("popup.ui"), &error) == 0
